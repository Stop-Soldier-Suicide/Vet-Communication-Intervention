...
#Reviewer Response Concern
```{r}
who.noone=subset(complete.data, Who.No.One == "1")
who.noone=who.noone %>%
  mutate(Total.who = rowSums(across(c(11:20))))
who.noone=subset(who.noone, Total.who > 1)

how.noone=subset(complete.data, How.Not.At.All == "1")
how.noone=how.noone %>%
  mutate(Total.how = rowSums(across(c(21:29))))
how.noone=subset(how.noone, Total.how > 1)

allow.not=subset(complete.data, Allow.Nothing == "1")
allow.not=allow.not %>%
  mutate(Total.allow = rowSums(across(c(11:20))))
allow.not=subset(allow.not, Total.allow > 1)
